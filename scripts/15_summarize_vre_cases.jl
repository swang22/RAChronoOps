...
function summarize_case(case_dir::String)::DataFrameRow
    ...
    stor_pow_mw  = m.storage_power_mw
    ...
    vre_cap_share_no_stor  = total_vre_cap / (therm_cap_mw + total_vre_cap)
    vre_cap_share_incl_stor= total_vre_cap / (therm_cap_mw + total_vre_cap + stor_pow_mw)
    ...
    mean_net_load          = mean(net_load_mw)
    peak_net_load          = maximum(net_load_mw)
    n_neg_net_load         = count(<(0.0), net_load_mw)
    vre_exceeds_load_h     = count(>(1e-6), vre_avail_mw .- load_df.load_mw)
    ...
    row = DataFrame(
        ...
        vre_capacity_share_no_storage  = round(vre_cap_share_no_stor;   digits = 4),
        vre_capacity_share_incl_storage= round(vre_cap_share_incl_stor; digits = 4),
        ...
        net_load_mean_mw               = round(mean_net_load;           digits = 1),
        net_load_peak_mw               = round(peak_net_load;           digits = 1),
        n_hours_negative_net_load      = n_neg_net_load,
        negative_net_load_hours        = n_neg_net_load,
        vre_exceeds_load_hours         = vre_exceeds_load_h,
        notes                          = m.notes,
    )
    return first(row)
end
...
let
    ...
    println("=" ^ 122)
    @printf("  %-20s %10s %10s %10s %10s %10s %10s %12s\n",
            "case_name",
            "VRE_cap_ns", "VRE_cap_is", "vre_e_shr",
            "neg_NL_h",   "NL_peak_mw",
            "wind_cap",   "solar_cap")
    @printf("  %-20s %10s %10s %10s %10s %10s %10s %12s\n",
            "", "(no stor)", "(w/ stor)", "(frac)",
    ...
    for r in eachrow(summary)
        @printf("  %-20s %10.4f %10.4f %10.4f %10d %10.1f %10.1f %12.1f\n",
                r.case_name,
                r.vre_capacity_share_no_storage,
                r.vre_capacity_share_incl_storage,
                r.available_vre_energy_share,
                r.negative_net_load_hours,
                r.net_load_peak_mw,
